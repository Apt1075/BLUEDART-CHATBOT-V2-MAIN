...
# ─── Queries jo date ke bina kaam karti hain (live/real-time) ────────────────
NO_DATE_NEEDED_TRIGGERS = [
    "running trips", "active trips", "current trips", "abhi chal rahe",
    "live trips", "trips running", "stopped vehicles", "stopped for",
    "halted for", "vehicle stopped", "max halt", "halt duration",
    "transshipment", "close trips today", "aaj kitne",
    "max trips", "most trips", "top vehicle", "stopped more than",
    "stopped less than", "vehicles stopped",
]

# ─── courier_trip_detail ke saare queryable fields ────────────────────────────
# Format: keyword_in_message → {field: value} or {field: operator}
FIELD_KEYWORD_MAP = {
    # shipment_method
    "sfc":              {"shipment_method": "SFC_Network"},
    "sfc_network":      {"shipment_method": "SFC_Network"},
    "sfc network":      {"shipment_method": "SFC_Network"},
    "air network":      {"shipment_method": "AIR_Network"},
    "air_network":      {"shipment_method": "AIR_Network"},
    "feeder":           {"shipment_method": "Feeder"},
    "pick up":          {"shipment_method": "Pick Up and Delivery"},
    "pickup":           {"shipment_method": "Pick Up and Delivery"},
    "comm/bz":          {"shipment_method": "COMM/BZ"},

    # region_code
    "north region":     {"region_code": "NORTH"},
    "south region":     {"region_code": "SOUTH"},
    "east region":      {"region_code": "EAST"},
    "west region":      {"region_code": "WEST"},
    "west2 region":     {"region_code": "WEST2"},
    ...
    # exception_common_backend (GPS main IMEI)
    "gps na":           {"exception_common_backend": "GPS NA"},
    "no connectivity":  {"exception_common_backend": "No Connectivity"},
    "gps active":       {"exception_common_backend": ""},

    # exception_common_backend_2 (Fixed lock)
    "fixed lock na":    {"exception_common_backend_2": "GPS NA"},
    "fixed lock inactive": {"exception_common_backend_2": "GPS NA"},
    "fixed lock active":{"exception_common_backend_2": ""},

    # exception_common_backend_3 (Portable lock)
    "portable lock na": {"exception_common_backend_3": "GPS NA"},
    "portable lock inactive": {"exception_common_backend_3": "GPS NA"},
    "portable lock active": {"exception_common_backend_3": ""},

    # imei_no_type (GPS vendor type)
    ...
    # ATD/ATA
    "atd not captured": {"actual_source_departure_time": "__empty__"},
    "atd missing":      {"actual_source_departure_time": "__empty__"},
    "ata not captured": {"actual_destination_arrival_time": "__empty__"},
    "ata missing":      {"actual_destination_arrival_time": "__empty__"},

    # close_by / close_remarks
    "forcefully closed":{"close_remarks": "__regex_supervisor__"},
    "supervisor close": {"close_remarks": "__regex_supervisor__"},
}
ANALYTICS_TRIGGERS = [
...
def should_use_history(msg: str, ids: dict) -> bool:
    """
    Use history IDs ONLY when:
    1. No IDs in current message → follow-up query
    2. Explicit context words present ("this trip", "that vehicle", etc.)
    DO NOT use when current message has its own shipment_no/vehicle_no.
    """
    has_ids = any(ids.get(k) for k in ["shipment_no", "vehicle_no", "imei"])
    if has_ids:
        # Fresh query with own IDs → only use history if explicit context word
        return any(kw in msg.lower() for kw in CONTEXT_WORDS)
    # No IDs → follow-up query, use history
...
def extract_ids_from_history(history: list) -> dict:
    ...
    # 15-digit first, then 7-9 digit
    m = re.search(r'\b(9\d{14})\b', all_text)
    if m:
        ids["shipment_no"] = m.group(1)
    else:
        m = re.search(r'\b(\d{7,9})\b', all_text)
        if m: ids["shipment_no"] = m.group(1)

    # Trip XXXXXXX pattern in HTML replies
    m = re.search(r'Trip\s+(\d{7,15})', all_text)
    if m and not ids["shipment_no"]:
        ids["shipment_no"] = m.group(1)

    # Vehicle no
    m = re.search(r'\b([A-Z]{2}\d{2}[A-Z]{1,3}\d{4})\b', all_text.upper())
    if m: ids["vehicle_no"] = m.group(1)

    # Region
    for region in ["NORTH","SOUTH","EAST","WEST","WEST2"]:
        if region.lower() in all_text.lower():
...
async def fetch_bulk_report(message: str) -> dict:
    ...
    elif any(x in msg for x in ["active trips","active only","running trips"]):     conditions["trip_status"] = 1
    ...
    if "gps active" in msg or "gps is active" in msg:        conditions["exception_common_backend"] = ""
    elif any(x in msg for x in ["gps inactive","gps na","gps is na"]): conditions["exception_common_backend"] = "GPS NA"
    elif "no connectivity" in msg:                            conditions["exception_common_backend"] = "No Connectivity"
...
def needs_date_clarification(msg: str) -> bool:
    """True = ask for date, False = proceed directly."""
    msg_l = msg.lower()

    # Live/real-time queries → no date needed
    if any(kw in msg_l for kw in NO_DATE_NEEDED_TRIGGERS):
        return False

    # Already has date reference
    date_words = [
        "today", "yesterday", "aaj", "kal", "last week", "last month",
        "this month", "this week", "last 7", "last 3", "last 30",
        "january", "february", "march", "april", "may", "june",
        "july", "august", "september", "october", "november", "december",
        "2026", "2025", "2024",
    ]
    has_date = (
        any(w in msg_l for w in date_words) or
        bool(re.search(r'\d{4}-\d{2}-\d{2}', msg)) or
        bool(re.search(r'last\s+\d+\s+days?', msg_l))
    )
    if has_date:
        return False

    # Any known filter keyword → need date
    if any(kw in msg_l for kw in FIELD_KEYWORD_MAP):
        return True

    return False


def extract_understood_filters(msg: str) -> list:
    """Return list of (label, value) tuples of detected filters."""
    msg_l  = msg.lower()
    found  = {}
    for kw, field_map in FIELD_KEYWORD_MAP.items():
        if kw in msg_l:
            for field, val in field_map.items():
                label = field.replace("_", " ").title()
                if val == "__empty__":     display = "Not present / Empty"
                elif val == "__exists__":  display = "Present"
                elif val == "__3rdparty__":display = "3rd Party GPS"
                elif val == "__regex_supervisor__": display = "Supervisor Close"
                else:                     display = str(val)
                found[label] = display
    return list(found.items())


def _date_range_table_rows() -> str:
    """Generate date range options with exact from-to dates."""
    now   = datetime.now()
    today = now.strftime("%Y-%m-%d")

    yesterday = (now - timedelta(days=1)).strftime("%Y-%m-%d")

    last7_from = (now - timedelta(days=7)).strftime("%Y-%m-%d")
    last30_from= (now - timedelta(days=30)).strftime("%Y-%m-%d")

    this_month_from = now.strftime("%Y-%m-01")

    last_month_last = (now.replace(day=1) - timedelta(days=1))
    last_month_from = last_month_last.replace(day=1).strftime("%Y-%m-%d")
    last_month_to   = last_month_last.strftime("%Y-%m-%d")
    last_month_name = last_month_last.strftime("%B %Y")

    this_month_name = now.strftime("%B %Y")

    # Current month name for april/may etc
    prev2 = (now.replace(day=1) - timedelta(days=1)).replace(day=1) - timedelta(days=1)
    prev2_name = prev2.strftime("%B %Y")
    prev2_from = prev2.replace(day=1).strftime("%Y-%m-%d")
    import calendar
    prev2_last = calendar.monthrange(prev2.year, prev2.month)[1]
    prev2_to   = prev2.replace(day=prev2_last).strftime("%Y-%m-%d")

    th = 'style="padding:9px 14px;text-align:left;font-size:11px;font-weight:600;color:#fff;background:#1e3a5f;white-space:nowrap"'
    td1= 'style="padding:9px 14px;border-bottom:1px solid #f3f4f6;color:#111827;font-size:12px;font-weight:500;white-space:nowrap"'
    td2= 'style="padding:9px 14px;border-bottom:1px solid #f3f4f6;color:#6b7280;font-size:11px;white-space:nowrap"'
    td3= 'style="padding:9px 14px;border-bottom:1px solid #f3f4f6;color:#2563eb;font-size:12px;font-family:monospace;white-space:nowrap"'

    options = [
        ("Today",                   f"{today} to {today}",                  "today"),
        ("Yesterday",               f"{yesterday} to {yesterday}",          "yesterday"),
        ("Last 7 days",             f"{last7_from} to {today}",             "last 7 days"),
        ("Last 30 days",            f"{last30_from} to {today}",            "last 30 days"),
        (f"This month ({this_month_name})", f"{this_month_from} to {today}","this month"),
        (f"Last month ({last_month_name})", f"{last_month_from} to {last_month_to}", "last month"),
        (f"{prev2_name}",           f"{prev2_from} to {prev2_to}",          prev2.strftime("%B").lower()),
        ("Custom date (single day)","YYYY-MM-DD to YYYY-MM-DD",             "2026-05-01"),
        ("Custom range",            "Any N days",                            "last 3 days"),
    ]

    header = f'<tr><th {th}>Period</th><th {th}>Date Range</th><th {th}>Type in chat</th></tr>'
    rows   = header
    for label, date_range, hint in options:
        rows += (
            '<tr>'
            + '<td ' + td1 + '>' + label      + '</td>'
            + '<td ' + td2 + '>' + date_range + '</td>'
            + '<td ' + td3 + '>' + hint       + '</td>'
            + '</tr>'
        )
    return rows
...
def build_clarification_html(msg: str) -> str:
    filters   = extract_understood_filters(msg)
    short_msg = msg[:50] + ("..." if len(msg) > 50 else "")
    rows      = _date_range_table_rows()
    ...
    for label, value in filters:
        pills_html += (
            ...
            'border-radius:20px;padding:3px 10px;font-size:11px;color:#1e40af;margin:2px">'
            + label + ': <b>' + value + '</b></span>'
        )
    if not pills_html:
        pills_html = '<span style="font-size:12px;color:#6b7280">Bulk trip query detected</span>'

    return (
        '<div style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;max-width:620px">'

        # Header
        '<div style="background:#1e3a5f;border-radius:12px 12px 0 0;padding:14px 18px">'
        '<div style="font-size:14px;font-weight:600;color:#fff;margin-bottom:4px">📅 Select Date Range</div>'
        '<div style="font-size:11px;color:rgba(255,255,255,.65)">'
        'Filters understood from your query:'
        '</div>'
        ...
        '<b>courier_trip_detail</b> has millions of records. '
        'A date range is required to keep queries fast and accurate. '
        'Select a period below or type it at the end of your message.'
        '</div>'

        # Date table
        '<div style="border-radius:8px;border:1px solid #e5e7eb;overflow:hidden;margin-bottom:14px;overflow-x:auto">'
        ...
        '<div style="font-size:11px;font-weight:600;color:#16a34a;margin-bottom:6px">💡 Example usage</div>'
        '<div style="font-size:11px;color:#374151;line-height:1.8">'
        'Append date to your message:<br>'
        '<span style="font-family:monospace;background:#dcfce7;padding:2px 8px;border-radius:4px;color:#15803d">'
        '"' + short_msg + ' <b>last month</b>"'
        '</span><br>'
        '<span style="font-family:monospace;background:#dcfce7;padding:2px 8px;border-radius:4px;color:#15803d">'
        '"' + short_msg + ' <b>last 7 days</b>"'
        '</span><br>'
        '<span style="font-family:monospace;background:#dcfce7;padding:2px 8px;border-radius:4px;color:#15803d">'
        '"' + short_msg + ' <b>2026-05-01</b>"'
        '</span>'
        '</div>'
        '</div>'
...
class ChatService:
    ...
    async def process(self, request: ChatRequest) -> ChatResponse:
        ...
        # ── 11. BULK REPORT ───────────────────────────────────────────────────
            ...
            # ── DATE CLARIFICATION CHECK ──────────────────────────────────────
            if needs_date_clarification(msg):
                reply = build_clarification_html(msg)
                ft    = round(time.time()-t1, 2)
                return ChatResponse(
                    reply=reply, intent=intent, query_type="GENERAL",
                    extracted_ids={k:v for k,v in ids.items() if v},
                    services_called=[],
                    fetch_time_seconds=ft, ai_time_seconds=0.0,
                    total_time_seconds=round(time.time()-start,2),
